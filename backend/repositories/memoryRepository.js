...
export const createMemory = (authorId, name, description,timestamp, friendIds, isPublic, pictureUrl = null) => {
  return new Promise((resolve, reject) => {
    ...
    db.serialize(() => {
      ...
      db.run(
        ...
        [id,authorId, name, description, timestamp, 1, isPublic ? 1 : 0, pictureUrl],
        function (err) {
          if (err) {
            db.run('ROLLBACK');
            return reject(err);
...
export const updateMemory = (id, userId, updates) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM memories WHERE id = ? AND user_id = ?', [id, userId], (err, row) => {
      ...
      const validUpdates = ['name', 'description', 'timestamp', 'is_public', 'picture_url'];
      const updateFields = [];
      const values = [];
...
export const updateFileInfo = (fileId, updates) => {
  return new Promise((resolve, reject) => {
    ...
    db.run(query, values, function(err) {
      if (err) return reject(err);
      resolve({ message: 'File info updated successfully', changes: this.changes });
    });
  });
};
...
export const updateMemoryParticipants = (memoryId, userId, friendIds) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run('DELETE FROM memory_participants WHERE memory_id = ? AND user_id != ?', [memoryId, userId], (err) => {
        if (err) {
          db.run('ROLLBACK');
          return reject(err);
        }
...
export const updateMemoryPictureUrl = (id, pictureUrl) => {
  return new Promise((resolve, reject) => {
    ...
    db.run(query, [pictureUrl, id], function(err) {
      if (err) return reject(err);
      resolve({ message: 'Memory picture URL updated successfully', changes: this.changes });
    });
  });
};
